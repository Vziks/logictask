...
public class InverseCase {
    ...
    private static String inverseWithArray(String srt) {
        ...
        for (int i = 0; i < chars.length; i++) {
            chars[i] = (
                    Character.isLetter(chars[i]) & Character.isLowerCase(chars[i]))
    ...
    private static String inverseWithStream(String str) {
        return str.chars().map(c -> (Character.isLetter(c) & Character.isLowerCase(c)) ? Character.toUpperCase(c) : Character.toLowerCase(c))
